...
app.post("/userdata", async (req, res) => {
  const user = await Users.find({ usertype: { $ne: "Admin" } }).sort({
    usertype: 1,
  });
  res.json(user);
});
...
app.post("/adminedituser", async (req, res) => {
  ...
  if (user) {
    const update = await Users.updateOne(
      { usertype, mail },
      { $set: { department, name, roll, year, section, phone, pphone, pmail } }
    );
...
app.post("/advoicerupdate", async (req, res) => {
  ...
  if (user) {
    const update = await Request.updateOne(
...
app.post("/advoicerhistory", async (req, res) => {
  ...
  const items = await Request.find({
    ...
    year,
    section,
    $or: [{ advoicerstatus: "accepted" }, { advoicerstatus: "rejected" }],
  }).sort({ _id: -1 });
  res.json(items);
});
...
app.post("/inchargehistory", async (req, res) => {
  ...
  const items = await Request.find({
    department,
    year,
    $or: [{ inchargestatus: "accepted" }, { inchargestatus: "rejected" }],
    advoicerstatus: "accepted",
  }).sort({ _id: -1 });
  res.json(items);
});
